...
export default function LeadDetailPage() {
  ...
  if (isLoading) return <p className="text-center text-on-surface-variant py-12">Loading...</p>;
  if (error || !lead) return <p className="text-error bg-error-container p-4 rounded-md">Lead tidak ditemukan.</p>;
  ...
  return (
    <>
      <Link href="/leads" className="m3-btn-text h-9 px-3 mb-4 inline-flex">
        <IconArrowLeft size={18} stroke={1.5} /> Kembali
      </Link>

      <div className="m3-card p-6 mb-6">
        <div className="flex justify-between items-start mb-4">
          <h1 className="text-headline-sm font-semibold text-on-surface">{lead.namaEo}</h1>
          {latestStatus && <StatusBadge status={latestStatus} />}
        </div>
        <div className="grid grid-cols-2 gap-4 text-body-md">
          <div><span className="text-label-md text-on-surface-variant">Tipe: </span>{lead.tipe?.nama ?? '-'}</div>
          <div><span className="text-label-md text-on-surface-variant">Alamat: </span>{lead.alamat}</div>
          <div><span className="text-label-md text-on-surface-variant">Speciality: </span>{lead.speciality ?? '-'}</div>
          <div><span className="text-label-md text-on-surface-variant">Sosmed: </span>{lead.linkSosmed ? <a href={lead.linkSosmed} target="_blank" rel="noopener noreferrer" className="text-primary">{lead.linkSosmed}</a> : '-'}</div>
        </div>
      </div>
      ...
      <div className="flex border-b border-outline-variant mb-6">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`px-5 py-3 text-title-sm -mb-px border-b-[3px] transition-colors ${
              activeTab === t.key
...
function ContactsTab({ contacts }: { contacts: ContactPerson[] }) {
  if (!contacts.length) return <p className="text-on-surface-variant text-center py-8">Belum ada contact person.</p>;
  return (
...
function StatusTab({ statuses }: { statuses: LeadStatus[] }) {
  if (!statuses.length) return <p className="text-on-surface-variant text-center py-8">Belum ada riwayat status.</p>;
...
function ActivitiesTab({ activities }: { activities: Activity[] }) {
  if (!activities.length) return <p className="text-on-surface-variant text-center py-8">Belum ada activity.</p>;
  ...
  return (
    <table className="m3-table">
      ...
      <tbody>{sorted.map((a) => <tr key={a.id}><td><span className="m3-badge bg-primary-container text-primary-on-container">{a.activityType}</span></td><td className="max-w-[300px] truncate">{a.notes}</td><td>{new Date(a.nextFollowUpDate).toLocaleDateString('id-ID')}</td><td>{a.creator?.nama ?? a.createdBy}</td></tr>)}</tbody>
    </table>
  );
}

function AssignmentTab({ assignments }: { assignments: Assignment[] }) {
  if (!assignments.length) return <p className="text-on-surface-variant text-center py-8">Belum ada assignment.</p>;
  ...
  return (
    <table className="m3-table">
      ...
      <tbody>{sorted.map((a) => <tr key={a.id}><td className="font-semibold">{a.pic?.nama ?? a.picId}</td><td><span className={`m3-badge ${a.isActive ? 'bg-g-100 text-g-500' : 'bg-surface-container-highest text-on-surface-variant'}`}>{a.isActive ? 'Aktif' : 'Tidak Aktif'}</span></td><td>{new Date(a.assignedAt).toLocaleString('id-ID')}</td><td>{a.reassignedNotes ?? '—'}</td></tr>)}</tbody>
    </table>
  );
}
