...
function renderDiff(prev: Record<string, unknown> | null, next: Record<string, unknown> | null): React.ReactNode {
  ...
  return (
    <div className="text-body-sm font-mono whitespace-pre-wrap break-words max-w-[400px]">
      <span className="text-error">{prevStr}</span>
      {prev && next ? <span className="text-on-surface-variant">{' → '}</span> : null}
      <span className="text-g-500">{nextStr}</span>
    </div>
  );
}
...
export default function AuditTrailTable({ auditTrails }: AuditTrailTableProps) {
  if (auditTrails.length === 0) {
    return <div className="py-6 text-center text-on-surface-variant">Belum ada audit trail.</div>;
  }

  return (
    <table className="m3-table">
      <thead>
        <tr><th>Entity</th><th>Perubahan</th><th>Diubah Oleh</th><th>Waktu</th></tr>
      </thead>
      <tbody>
        {auditTrails.map((trail) => (
          <tr key={trail.id}>
            <td><span className="m3-badge bg-tertiary-container text-tertiary-on-container">{trail.entityName}</span></td>
